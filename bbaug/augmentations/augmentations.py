...
#--------------------------------------------------------------
def zoom_scale(magnitude: float) -> iaa.Affine:
    """
    Scale/Zoom the image

    :magnitude (float): scale of the zoom in.
    :return (iaa.Affine): method to apply scaling
    """
    #dummy line just to indicate that in this case the magnitude will bring the scale
    scale_ = magnitude 
    return iaa.Affine(
        scale=(scale_),
        order=1, #1: cv2.INTER_LINEAR = Bi-linear
        backend="cv2"
    )
#--------------------------------------------------------------
