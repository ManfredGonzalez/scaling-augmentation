...
def evaluate_coco(img_path, image_ids, coco, model, conf_threshold, input_sizes, compound_coef, use_cuda, nms_threshold):
    results = []
    use_float16 = False
...
def run_metrics(compound_coef, nms_threshold, use_cuda, use_float16, 
    ...
    for img_id in image_ids:
        ...
        coco_annotations = coco_gt.loadAnns(ann_ids)
        ...
        #coco_annotation = coco_gt.loadAnns(ann_ids)
        # image name for input image
        #imgName = coco_gt.loadImgs(img_id)[0]['file_name']
    ...
    for conf_thres in confidence_list:
        ...
        if override_prev_results or not os.path.exists(f'results/{SET_NAME}_bbox_results.json'):
            ...
            pineapples_detected = evaluate_coco(VAL_IMGS, image_ids, coco_gt, model, conf_thres, input_sizes, compound_coef, use_cuda, nms_threshold)  
    
        #evaluate model using the ground truth and the predicted bounding boxes
        if(len(pineapples_detected) > 0):
            p,r,ap = mean_average_precision(pineapples_detected, ground_truth_boxes, iou_threshold=nms_threshold, box_format="corners", num_classes=1)
            p = p.item()
            r = r.item()
            ap = ap.item()
        ...
        with open(f'results/{params["project_name"]}_results_d{compound_coef}.csv', "a") as myfile:
            my_writer = csv.writer(myfile, delimiter=',', quotechar='"')
            my_writer.writerow([iteration, groundtruth_num, len(pineapples_detected), nms_threshold, conf_thres, p, r, f1_result])
    #--------------------------------------------
